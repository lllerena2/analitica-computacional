...
describir_vector <- function(str_vector){
  ...
  for (i in 1:(length(vector) - 1)) {
    ...
    print(matriz_transicion[estado_actual, estado_siguiente])
    matriz_transicion[estado_actual, estado_siguiente] <- matriz_transicion[estado_actual, estado_siguiente] + 1
  ...
  #output
  resultados <- list("num_ceros" = n_ceros, "num_unos" = n_unos, "max_racha_ceros" = max_ceros,"max_racha_ceros" =max_unos )
  return(matriz_transicion)
}

resultados = describir_vector(v)

#resultados$num_ceros
```


```{r}
# Supongamos que tienes un vector de observaciones de estados
observaciones <- c("A", "B", "A", "A", "B", "C", "C", "A", "B", "A")

# Función para calcular la matriz de transición a partir de las observaciones
calcular_matriz_transicion <- function(observaciones) {
  # Obtener los estados únicos y contar las transiciones
  estados <- unique(observaciones)
  n_estados <- length(estados)
  matriz_transicion <- matrix(0, nrow = n_estados, ncol = n_estados, dimnames = list(estados, estados))
  
  # Contar las transiciones entre estados
  for (i in 1:(length(observaciones) - 1)) {
    estado_actual <- observaciones[i]
    estado_siguiente <- observaciones[i + 1]
    matriz_transicion[estado_actual, estado_siguiente] <- matriz_transicion[estado_actual, estado_siguiente] + 1
  }
  
  # Normalizar las frecuencias para obtener probabilidades de transición
  # matriz_transicion <- matriz_transicion / rowSums(matriz_transicion)
  
  return(matriz_transicion)
}

# Calcular la matriz de transición
matriz_transicion <- calcular_matriz_transicion(observaciones)
